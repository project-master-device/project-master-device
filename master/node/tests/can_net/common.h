...
int test_send_msg(uint32_t hwa, uint16_t id, uint8_t smb, uint8_t port, int msg_type, can_net_send_callback_t cb) {
	...
	//rc =
	can_net_start_sending_msg(&msg, /*port,*/ cb);
	free(msg.data.itself);
	return rc;
}

int inline test_cmp_msg(msg_lvl2_t* msg, uint32_t hwa, uint16_t id, uint8_t smb, int msg_type) {
...
void test_send_callback(const int rc, msg_lvl2_t* msg) {
	printf("\n send_callback: rc=%d\n", rc);
	test_print_msg(msg);
}
void test_recv_callback(msg_lvl2_t* msg) {
	printf("\n receive_callback: \n");
	test_print_msg(msg);
}
